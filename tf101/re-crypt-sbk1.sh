...
key=1682ccd88a1a43eaa532eeb6ecfe1d98 # sbk v1
...
echo $bootloaderHash	| xxd -r -p | dd conv=notrunc of=tmp_bct.bin seek=2736 bs=1
echo $bootloaderLength 	| xxd -r -p | dd conv=notrunc of=tmp_bct.bin seek=2720 bs=1
...
dd if=/dev/zero of=tmp_bct_block.bin bs=1 count=4096
#put hash in Image
...
#append SBK to BCT
echo $key 		| xxd -r -p | dd conv=notrunc of=tmp_bct_block.bin seek=4080 bs=1
